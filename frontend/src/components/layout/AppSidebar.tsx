...
import { Home, AlertCircle, ListTodo, Bell, Settings, X, Edit, User } from 'lucide-react';
...
{ label: 'Report Issue', icon: AlertCircle, path: '/report', type: 'link' as const },
{ label: 'ISSUE MANAGEMENT', type: 'section' as const },
{ label: 'My Issues', icon: ListTodo, path: '/issues', type: 'link' as const },
{ label: 'Notifications', icon: Bell, path: '/notifications', type: 'link' as const },
{ label: 'SYSTEM', type: 'section' as const },
{ label: 'Settings', icon: Settings, path: '/settings', type: 'link' as const }];
      ...
      <>
          ...
          className="fixed left-0 top-0 bottom-0 z-50 w-[260px] glass rounded-r-2xl shadow-card-hover flex flex-col">
            ...
            <div className="px-5 pt-6 pb-2 flex items-center gap-3">
              ...
              <div>
                <h2 className="font-bold text-foreground tracking-tight text-base">CIVIQ</h2>
                <p className="text-muted-foreground tracking-wide text-xs font-normal">Smart Community Platform</p>
              </div>
            </div>
            ...
            <nav className="flex-1 px-3 pt-4 space-y-0.5 overflow-y-auto">
              ...
              if (item.type === 'header' || item.type === 'subtitle') return null;
              if (item.type === 'section') {
                return (
                  <p key={i} className="text-[10px] font-semibold text-muted-foreground tracking-widest uppercase px-3 pt-5 pb-1.5">
                      {item.label}
                    </p>);
